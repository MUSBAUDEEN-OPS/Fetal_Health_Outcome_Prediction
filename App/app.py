...
class FetalHealthPredictor:
    ...
    def preprocess_input(self, input_data):
        ...
        Fills missing features with median values from normal ranges.
        ...
        # Fill missing features with median of normal range
...
def render_manual_input():
    ...
    st.markdown("""
        <div class='info-card'>
            <strong>Instructions:</strong> Enter the available cardiotocography measurements below. 
            Missing or unknown values will be filled with default values. Hover over the info icons for guidance on normal ranges.
        </div>
    """, unsafe_allow_html=True)
...
def render_csv_upload():
    ...
    st.markdown("""
        <div class='info-card'>
            ...
            The file can include any subset of features - missing features will be filled with default values.
        </div>
    """, unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        try:
            ...
            # Check which features are present (case-insensitive)
            present_features = []
            missing_features = []
            ...
            # Rename columns to match expected feature names (case-sensitive)
            df_renamed = df.copy()
            for feature in present_features:
                actual_col_name = column_mapping[feature.lower()]
                if actual_col_name != feature:
                    df_renamed.rename(columns={actual_col_name: feature}, inplace=True)
            
            # Show information about missing features
            if missing_features:
                st.info(f"""
                    ℹ️ **Missing features ({len(missing_features)}):** {', '.join(missing_features[:5])}{'...' if len(missing_features) > 5 else ''}
                    
                    These will be filled with default values (0) for prediction.
                """)
            
            if present_features:
                st.success(f"✅ Found {len(present_features)} out of {len(FEATURE_INFO)} required features.")
            
            # Return first row as input data for prediction
            return df_renamed.iloc[0].to_dict()
...
def render_prediction_results(prediction_result, patient_info):
    ...
    fig.update_layout(
        title={
            ...
            'font': {'size': 18, 'color': '#2d3748', 'family': 'IBM Plex Sans'}
        },
        xaxis={
...
def render_feature_importance(input_data):
    ...
    fig.update_layout(
        ...
        xaxis={'title': 'Importance Score', 'range': [0, 1.1]},
        yaxis={'title': ''},
        height=500,
        plot_bgcolor='rgba(0,0,0,0)',
        paper_bgcolor='rgba(0,0,0,0)',
